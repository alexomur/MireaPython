...
def best_path(check, graph, coins, start):
    ...
    Для каждой монетки:
      ...
      - Считается бонус за монетное скопление – суммируется значение монеток в соседних клетках.
      - Вычисляется штраф, если противник может добраться до монетки быстрее:
            Для каждого противника, обнаруженного в графе (где check("player", ...) истинен),
            вычисляется кратчайший путь от него до монетки. Если ближайший противник достигает
            монетки быстрее, то штрафной коэффициент = (расстояние противника / наше расстояние),
            иначе коэффициент равен 1.
      - Окончательная оценка (score) = (ценность монетки + бонус) / ((расстояние + 1) * (1 + опасность)) * (штрафной коэффициент)

    Если ни один вариант не найден, выбирается резервное движение – ближайшая доступная клетка.
    ...
    while remaining_coins:
        ...
        for coin_pos, coin_value in remaining_coins.items():
            ...
            if opponents:
                ...
                if opp_distances:
                    ...
                    # Если противник может добраться быстрее, коэффициент меньше 1
                    if min_opp_distance < distance:
                        penalty = min_opp_distance / distance
                    else:
                        penalty = 1.0

            # Итоговая оценка кандидата: чем выше значение и чем меньше путь/опасность, тем лучше
            score = total_value / ((distance + 1) * (1 + danger_count)) * penalty
        ...
        if candidate_paths:
            ...
            # Если оценка слишком низкая, лучше прекратить сбор (нет смысла тратить время)
            if best_score <= 0.001:
                break
            route.extend(best_segment[1:])  # добавляем путь, избегая дублирования текущей позиции
            current = best_coin
            del remaining_coins[best_coin]
        else:
            # Резервный вариант: выбираем ближайшее доступное движение, даже если там есть игрок
            fallback_moves = []
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
